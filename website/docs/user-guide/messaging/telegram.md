...
sidebar_position: 1
title: "Telegram"
description: "Set up Meridian Agent as a Telegram bot"
---

# Telegram Setup

Meridian Agent integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "Meridian Agent") — this can be anything
...
Meridian Agent uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
Meridian Agent works in Telegram group chats with a few considerations:
