...
sidebar_position: 4
title: "Slack"
description: "Set up Meridian Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Meridian Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
...
4. Enter an app name (e.g., "Meridian Agent") and select your workspace
5. Click **Create App**
...
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Meridian Agent what time is it?`). In channels, Hermes replies in a thread attached to that message. |
...
Make sure the bot has been **invited to the channel** (`/invite @Meridian Agent`).
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Meridian Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Meridian Agent` |
...
7. ✅ Bot was **invited** to the channel (`/invite @Meridian Agent`)
