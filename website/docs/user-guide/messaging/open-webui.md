...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to Meridian Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With Meridian Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
Open WebUI connects to Meridian Agent's API server just like it would connect to OpenAI. Hermes handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.
...
- model name advertised to Open WebUI: `Meridian Agent`
...
HERMES_API_MODEL_NAME='My Meridian Agent' \
bash scripts/setup_open_webui.sh
```
...
### 2. Start Meridian Agent gateway
...
# {"object":"list","data":[{"id":"meridian-agent", ...}]}
```
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and Meridian Agent responds accordingly. Each request includes the full conversation history.
...
With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and Meridian Agent can preserve full tool call history across turns via `previous_response_id`. When `stream: true`, Hermes also streams spec-native `function_call` and `function_call_output` items, which enables custom structured tool-call UI in clients that render Responses events.
...
2. Meridian Agent creates a server-side `AIAgent` instance using the API server's profile, model/provider config, memory, skills, and configured API-server toolsets
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | Meridian Agent's API URL (include `/v1`) |
...
Meridian Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in Meridian Agent.
