# ============================================================================
# Meridian Agent Installer for Windows
# ============================================================================
...
function Write-Banner {
    ...
    Write-Host "|             * Meridian Agent Installer                  |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Install-Git {
    ...
    which Meridian uses to run shell commands.
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Meridian needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Meridian copy of Git ourselves is predictable and
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Meridian features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Meridian needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    Write-Warn "Could not locate bash.exe -- Meridian may not find Git Bash."
...
function Test-Node {
    ...
    if (Test-Path $managedNode) {
        ...
        $env:Path = "$HermesHome\node;$env:Path"
        Write-Success "Node.js $version found (Meridian-managed)"
        $script:HasNode = $true
        return $true
    }
    ...
    if (Test-Path $InstallDir) {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                Write-Info "terminals, running meridian processes) and try again."
                throw
            }
        }
    }
...
"@ 2>$null
    ...
    # users hit and lazy-import errors from `meridian dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `meridian dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `meridian dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up meridian command..."
    ...
    # Add the venv Scripts dir to user PATH so meridian is globally available.
    # On Windows, meridian.exe in venv\Scripts\ has the venv Python baked in.
    ...
    Write-Success "meridian command ready"
}
...
Edit this to customize how Meridian communicates with you.
...
function Install-NodeDeps {
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'meridian setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'meridian setup'."
        return
    }
    ...
    # Run meridian setup using the venv Python directly (no activation needed)
    if (-not $NoVenv) {
...
function Start-GatewayIfConfigured {
    ...
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        ...
        Write-Info "Running 'meridian whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: meridian gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            ...
            Write-Warn "Failed to start gateway. Run manually: meridian gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: meridian gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host ""
    Write-Host "   meridian            " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   meridian setup      " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   meridian config     " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   meridian config edit" -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   meridian gateway    " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   meridian update     " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Meridian repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
    ...
    @{ Name = "path";             Title = "Adding Meridian to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
