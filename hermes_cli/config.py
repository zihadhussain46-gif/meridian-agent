"""
Configuration management for Meridian Agent.
...
- meridian config          - Show current configuration
- meridian config edit     - Open config in editor
- meridian config set      - Set a specific value
- meridian config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``meridian logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  meridian config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
def get_container_exec_info() -> Optional[dict]:
    ...
    container_name = info.get("container_name", "meridian-agent")
    exec_user = info.get("exec_user", "meridian")
...
DEFAULT_CONFIG = {
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # meridian/cache/piper-voices/
...
OPTIONAL_ENV_VARS = {
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. # meridian)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'meridian-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
