...
def auth_add_command(args) -> None:
    ...
    raise SystemExit(f"`meridian auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `meridian auth status spotify`.")
