...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Meridian runs shell commands and code.")
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Meridian delivers cron job results,")
...
def _write_slack_manifest_and_instruct():
    ...
    try:
        ...
        manifest = _build_full_manifest(
            bot_name="Meridian",
            bot_description="Your Meridian agent on Slack",
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where Meridian delivers cron job results and notifications.")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Meridian to iMessage via BlueBubbles — a free, open-source")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Meridian from anywhere.")
...
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Meridian to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Meridian at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Meridian at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Meridian at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Meridian at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Meridian equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Meridian config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Meridian.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Meridian's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Meridian can preview what would be imported before making any changes.")
    print()
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Meridian (use hermes claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    print(color("│     ⚕ Meridian Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Meridian Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    if is_existing:
        ...
        print_success("You already have Meridian configured.")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Meridian?",
            [
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Meridian to messaging apps to chat from anywhere.")
