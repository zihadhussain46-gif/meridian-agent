...
configured for "cli" in `meridian tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HERMES_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `meridian chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    - MERIDIAN_INFERENCE_MODEL / HERMES_INFERENCE_MODEL
    - MERIDIAN_INFERENCE_PROVIDER / HERMES_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"meridian -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "meridian -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"meridian -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "meridian -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "meridian -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to MERIDIAN_INFERENCE_MODEL / HERMES_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            MERIDIAN_INFERENCE_PROVIDER / HERMES_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "meridian -z: --provider requires --model (or MERIDIAN_INFERENCE_MODEL / HERMES_INFERENCE_MODEL). "
