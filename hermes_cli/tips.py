...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or meridian -c.",
    ...
    "Ctrl+Z suspends Meridian to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "meridian -c resumes your most recent CLI session. meridian -c \"project name\" resumes by title.",
    "meridian -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "meridian -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "meridian chat -t web,terminal enables only specific toolsets for a focused session.",
    "meridian chat -s github-pr-workflow preloads a skill at launch.",
    "meridian chat -q \"query\" runs a single non-interactive query and exits.",
    "meridian chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "meridian chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "meridian --yolo bypasses all dangerous command approval prompts for the entire session.",
    "meridian chat --source telegram tags the session for filtering in meridian sessions list.",
    "meridian -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "meridian doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "meridian dump outputs a compact setup summary — great for bug reports.",
    "meridian config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "meridian config edit opens config.yaml in your default editor.",
    "meridian config check scans for missing or stale configuration options.",
    "meridian sessions browse opens an interactive session picker with search.",
    "meridian sessions stats shows session counts by platform and database size.",
    "meridian sessions prune --older-than 30 cleans up old sessions.",
    "meridian skills search react --source skills-sh searches the skills.sh public directory.",
    "meridian skills check scans installed hub skills for upstream updates.",
    "meridian skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "meridian skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "meridian mcp add github --command npx adds MCP servers from the command line.",
    "meridian mcp serve runs Meridian itself as an MCP server for other agents.",
    "meridian auth add lets you add multiple API keys for credential pool rotation.",
    "meridian completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "meridian logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "meridian backup creates a zip backup of your entire Meridian home directory.",
    "meridian profile create coder creates an isolated profile that becomes its own command.",
    "meridian profile create work --clone copies your current config and keys to a new profile.",
    "meridian update syncs new bundled skills to ALL profiles automatically.",
    "meridian gateway install sets up Meridian as a system service (systemd/launchd).",
    "meridian memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "meridian webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: meridian tools disables unused tools, meridian skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "meridian models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as meridian -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Meridian tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'meridian profile create coder' creates the 'coder' command.",
    "meridian profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Meridian prints a resume command with session ID and stats.",
    "meridian sessions export backup.jsonl exports all sessions for backup or analysis.",
    "meridian -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "meridian skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: meridian cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Meridian runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "meridian gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Meridian via a local macOS server.",
    ...
    "Meridian loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "meridian chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "meridian plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "meridian acp runs Meridian as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "meridian login supports OAuth-based auth for Nous and OpenAI Codex providers.",
    ...
    "meridian status --deep runs deeper diagnostic checks across all components.",
    ...
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Meridian your own.",
    ...
    "Before a gateway session reset, Meridian auto-flushes important facts to memory in the background.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'meridian profile create ls' would be rejected.",
    "meridian profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "meridian claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via meridian tools.",
    ...
    '/goal <text> sets a standing Ralph-loop objective — Meridian auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Meridian config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Meridian is working.',
    ...
    # --- Curator ---
    'meridian curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "meridian curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'meridian curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'meridian auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'meridian dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'meridian dashboard --tui embeds the full Meridian TUI in your browser via xterm.js and a WebSocket PTY.',
    ...
    'meridian -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'meridian chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'meridian chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'meridian chat --ignore-user-config skips ~/.hermes/config.yaml — reproducible bug reports and CI runs.',
    "meridian chat --source tool tags programmatic chats so they don't clutter meridian sessions list.",
    'meridian dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'meridian sessions rename <ID> "new title" renames any past session; meridian sessions delete <ID> removes one.',
    'meridian import restores a session export or profile archive produced by sessions export or profile export.',
    'meridian fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'meridian pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'meridian setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'meridian status --deep runs the full health sweep across every component; plain meridian status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Meridian block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from meridian sessions list by default — set --source explicitly to see them.',
